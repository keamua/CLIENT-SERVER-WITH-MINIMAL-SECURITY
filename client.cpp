...
{ 
	...
	char recvBuf[1010]; 
	char sendBuf[1010]; 
	char *rBuf = (char*)malloc(1010); 
	char *sBuf = (char*)malloc(1010); 
	*/
	...
	//接受PASS_REQ
	cout<<sendBuf<<endl;
	recvfrom(sockCli, recvBuf, 1010, 0, (SOCKADDR*)&addrSrv, &len);
	...
	while( recvBuf[0] != 0){
		...
		case PASS_RESP:{
			...
			if(strcmp(password,pkt_pwd.PWD)){
				...
				sendto(sockCli,sendBuf,26,0,(SOCKADDR*)&addrSrv,len); 
			}
			if(passcount > 3){
